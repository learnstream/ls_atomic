...
class Quiz < ActiveRecord::Base
  ...
  scope :locked, lambda { |course_id, user_id| select('distinct quizzes.*, memories.due, memories.views').where('quizzes.course_id = ?', course_id).joins(:components => :memories).where('memories.user_id = ?', user_id).merge(Memory.where('memories.views = 0')) }
  ...
  scope :due, lambda { |course_id, user_id| select('distinct quizzes.*, memories.due').where('quizzes.course_id = ?', course_id).joins(:components => :memories).where('memories.user_id = ?', user_id).merge(Memory.due_before(DateTime.now.utc)) } 
